...
class TestSquarePuzzleBoard(unittest.TestCase):
    ...
    def test_valid_moves(self):
        ...
        board._square_positions = np.array([i for i in range(1, n + 1)] + [0] + [i for i in range(n + 1, n ** 2)]).reshape((n, -1))
        self.assertEqual(board.valid_moves(), [(0, 0), (2, 0), (1, 1)])

        board._square_positions = np.array([i for i in range(1, n)] + [0] + [i for i in range(n, n ** 2)]).reshape((n, -1))
        self.assertEqual(board.valid_moves(), [(1, 3), (0, 2)])

        board._square_positions = np.array([i for i in range(1, n + 2)] + [0] + [i for i in range(n + 2, n ** 2)]).reshape(
            (n, -1))
    ...
    def test_move_valid_move(self):
        ...
        # valid move, board shoul have moved
